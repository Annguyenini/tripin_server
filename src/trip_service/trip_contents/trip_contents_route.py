...
class TripContentsRoute(RouteBase):
    ...
    def _register_route(self):
        ...
        self.bp.route("/location-conditions", methods=["GET"])(
            self.request_current_location_condition
        )
        ...
        self.bp.route("/<token>/coordinates-by-token", methods=["GET"])(
            self.get_trip_coors_by_token
        )
        self.bp.route("/<token>/medias-by-token", methods=["GET"])(
            self.get_trip_medias_by_token
        )
    ...
    def add_coordinates(self, trip_id):
        ...
        owner_validation = self.trip_data_base_service.trip_owner_validation(
        ...
        if not owner_validation:
            return jsonify(
                {
                    "code": "not authorize",
                    "message": "Your account are not authorize to modified this trip",
                }
            ), 401
        ...
        insert, db_version = self.trip_contents_service.insert_coordinates_to_db(
            trip_id=trip_id, coordinates=coordinates
        )
                ...
                return jsonify(
                    {
                        ...
                        "message": f"Current version: {db_version} make sure to have the version that higher than this!",
                        "missing_versions": db_version,
                    }
                ), 409
            return jsonify(
                {"code": "failed", "message": "Failed to save to database"}
            ), 500

        return jsonify(
            {"code": "successfully", "message": "Successfully store into database"}
        ), 200
    ...
    def media_upload(self, trip_id):
        ...
        validation = self.trip_data_base_service.trip_owner_validation(
            user_id=user_id, trip_id=trip_id
        )
        if not validation:
            return jsonify(
        ...
        if image:
            # handle image upload
            path = image.filename
            upload_status, hash = self.trip_contents_service.upload_media(
                "photo",
                path=path,
                media=image,
                trip_id=int(trip_id),
        ...
        if not upload_status:
            return jsonify(
                {"code": ERROR_KEYS.FAILED, "message": ERROR_MESSAGE.SERVER_FAILED}
            ), 500
    ...
    def delete_media(self):
        ...
        if not media_id:
            return {
                "code": "no_media_id",
                "message": "No media_id or invalid value type was send!",
            }
        ...
        # pocess delete
        delete_media, error_delete = self.trip_contents_service.delete_media(
            media_id=media_id, trip_id=trip_id, modified_time=modified_time
        )
        if not delete_media:
            print(error_delete)
        ...
        return (
            {
                "code": "successfully",
                "message": "Media delete from server successfully!",
                "hash": hash,
            }
        ), 200
    ...
    def delete_coordinate(self):
        ...
        if not coordinate_id:
            return {
                "code": "no_coordinate_id",
                "message": "No coordinate_id or invalid value type was send!",
            }
        ...
        # pocess delete
        delete_coordinate, error_delete = self.trip_contents_service.delete_trip_coord(
            coordinate_id=coordinate_id, modified_time=modified_time
        )
        if not delete_coordinate:
            return (error_delete), 500
        hash = self.trip_data_base_service.generate_trip_coordinate_hash(
            trip_id=trip_id
        )
        return (
            {
    ...
    def request_current_location_condition(self):
        ...
        if longitude and latitude:
            geo_data = self.geo_service.get_geo_data(
    ...
    def get_trip_coors(self, trip_id: int):
        ...
        user_id = user_data["user_id"]
        authorize = self.trip_data_base_service.trip_owner_validation(
            user_id=user_id, trip_id=trip_id
        )
        if not authorize:
            return jsonify(
                {
                    "code": "not authorize",
                    "message": "You are not authorize to get this trip data!",
                }
            ), 403

        # get client version from header for cache comparison
        client_hash = request.headers.get("If-None-Match")
        server_hash = self.trip_data_base_service.generate_trip_coordinate_hash(
            trip_id=trip_id
        )
        ...
        return jsonify(
            {"message": "Successfully", "coordinates": coors, "hash": server_hash}
        ), 200
    ...
    def get_trip_coors_by_token(self, token):
        ...
        trip_data = self.trip_data_base_service.get_trip_data_by_shared_token(
            token=token
        )
        ...
        if not coors:
            return jsonify(
                {"code": "failed", "message": "Failed to get coordinates"}
            ), 500
    ...
    def get_trip_medias(self, trip_id: int):
        ...
        client_hash = request.headers.get("If-None-Match")
        # print(client_hash)
        server_hash = self.trip_data_base_service.generate_trip_media_hash(
            trip_id=trip_id
        )
        ...
        return jsonify(
            {"message": "Successfully", "medias": medias, "hash": server_hash}
        ), 200

...
class ContentsSyncRoute(RouteBase):
    ...
    def _register_route(self):
        self.bp.route("/trip-coordinate-hash", methods=["POST"])(
            self.get_trip_coordinate_hash
        )
        self.bp.route("/trip-medias-hash", methods=["POST"])(self.get_trip_medias_hash)
        self.bp.route("/trip-medias-metadata", methods=["POST"])(
            self.get_trip_media_metadata
        )
    ...
    def get_trip_coordinate_version(self):
        ...
        coordinates_version = self.tripDataBaseService.get_trip_contents_version(
            trip_id=trip_id, version_type=DATABASEKEYS.TRIPS.TRIP_COORDINATES_VERSION
        )
    ...
    def get_trip_coordinate_hash(self):
        ...
        trip_id = request.json["trip_id"]
        trip_validation = self.tripDataBaseService.trip_owner_validation(
            user_id=user_id, trip_id=trip_id
        )
        if not trip_validation:
            return jsonify(
                {"code": ERROR_KEYS.NOPERMISSION, "message": ERROR_MESSAGE.NOPERMISSION}
            ), 403
        coordinate_hash = self.tripDataBaseService.generate_trip_coordinate_hash(
            trip_id=trip_id
        )

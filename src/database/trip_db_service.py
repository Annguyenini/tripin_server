...
class TripDatabaseService(Database):
    ...
    def insert_to_database_trip(self, user_id: int, trip_name: str, imageUri: str):
        ...
        try:
            cur.execute(
                f"INSERT INTO {DATABASEKEYS.TABLES.TRIPS} (user_id,trip_name,created_time, active,image) VALUES (%s,%s,%s,%s,%s) RETURNING id",
                (user_id, trip_name, created_time, True, imageUri),
            )
    ...
    def insert_media_into_db(
        ...
        try:
            con, cur = self.connect_db()
            cur.execute(
                f"""INSERT INTO {DATABASEKEYS.TABLES.TRIP_MEDIAS} (
            ...
            self.ErrorHandler.logger("TripDatabase").error(
                "Failed to insert Media into database", body=e
            )
        finally:
            self.close_db(conn=con)
    ...
    def update_all_trips_version(self, user_id):
        ...
        try:
            cur.execute(
                f"UPDATE {DATABASEKEYS.TABLES.USERDATA} SET trips_data_version = trips_data_version+1 WHERE id = %s",
                (user_id,),
            )
            con.commit()
            ...
            self.ErrorHandler.logger("TripDataBase").error(
                "Failed at update all trips version", body=e
            )
            return False
        finally:
            self.close_db(conn=con)

    def update_trip_version(
        self, type_of_version: str, trip_id: int, version: int = None
    ):
        allow_type = [""]
        con, cur = self.connect_db()
        ...
        try:
            con, cur = self.connect_db()
            cur.execute(
                f"UPDATE {DATABASEKEYS.TABLES.TRIPS} SET {type_of_version} = {type_of_version}+1 WHERE id = %s",
                (trip_id,),
            )
            con.commit()
            return True if cur.rowcount >= 1 else False
    ...
    def get_user_trips_data(self, user_id: int, data_type: str) -> any:
        ...
        try:
            cur.execute(
                f"SELECT {data_type} FROM {DATABASEKEYS.TABLES.USERDATA} WHERE id = %s",
                (user_id,),
            )
            con.commit()
            ...
            self.ErrorHandler.logger("TripDataBase").error(
                "Failed at get user trips data", body=e
            )
            return False
        finally:
            self.close_db(conn=con)
    ...
    def get_trip_contents_version(self, trip_id: int, version_type: str) -> int:
        ...
        try:
            cur.execute(
                f"SELECT {version_type} FROM {DATABASEKEYS.TABLES.TRIPS} WHERE {DATABASEKEYS.TRIPS.TRIP_ID} = %s",
                (trip_id,),
            )
            con.commit()
            version = cur.fetchone()
            ...
            self.ErrorHandler.logger("TripDataBase").error(
                "Failed to get trip content version", body=e
            )
            return False
        finally:
            self.close_db(conn=con)
    ...
    def get_user_id_from_trip_id(self, trip_id: int) -> int:
        try:
            con, cur = self.connect_db()
            cur.execute(
                f"SELECT {DATABASEKEYS.TRIPS.USER_ID} FROM {DATABASEKEYS.TABLES.TRIPS} WHERE {DATABASEKEYS.TRIPS.TRIP_ID} = %s",
                (trip_id,),
            )
            con.commit()
            ...
            self.ErrorHandler.logger("TripDataBase").error(
                "Failed to get user id from trip id", body=e
            )
            return False
        finally:
            self.close_db(conn=con)
    ...
    def get_trip_coordinates(self, trip_id: int):
        ...
        try:
            ...
            cur.execute(
                f"""SELECT * FROM {DATABASEKEYS.TABLES.TRIP_COORDINATES}
                        WHERE {DATABASEKEYS.TRIP_COORDINATES.TRIP_ID} = %s
                        ORDER BY {DATABASEKEYS.TRIP_COORDINATES.COORDINATES_ID} ASC""",
                (trip_id,),
            )
            con.commit()
            coors = cur.fetchall()
            ...
            self.ErrorHandler.logger("TripDataBase").error(
                "Failed to get trip coordinate", body=e
            )
            return False
        finally:
            if con:
    ...
    def trip_owner_validation(self, user_id: int, trip_id: int) -> bool:
        try:
            ...
            self.ErrorHandler.logger("TripDataBase").error(
                "Failed to validate trip owner", body=e
            )
            return False
    ...
    def generate_trip_coordinate_hash(self, trip_id: int):
        ...
        try:
            ...
            self.ErrorHandler.logger("TripDataBase").error(
                "Failed to get trip coordinate max", body=e
            )
            return None
        finally:
            if con:

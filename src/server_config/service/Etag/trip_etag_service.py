...
class TripEtagService(EtagService):
    ...
    def regenerate_user_trips_etag_handler(self, user_id: int) -> str:
        ...
        # new new etag
        new_version = self.trip_database_service.get_user_trips_data(
            user_id=user_id, data_type=DATABASEKEYS.USERDATA.TRIPS_DATA_VERSION
        )
        new_etag_data = self.get_all_trip_etag_data_string(
            user_id=user_id, version=new_version
        )
        ...
        self.trip_database_service.update_db(
            ...
            value=user_id,
            item_to_update=DATABASEKEYS.USERDATA.TRIPS_DATA_ETAG,
            value_to_update=new_etag,
        )

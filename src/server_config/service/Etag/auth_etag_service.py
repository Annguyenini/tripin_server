...
class AuthEtagService(EtagService):
    ...
    def store_userdata_etag_to_DB_handler(self, user_id, etag: str):
        put_to_db = self.databaseService.update_db(
            DATABASEKEYS.TABLES.USERDATA,
    ...
    def get_userdata_etag_from_database(self, user_id: int):
        ...
        cur.execute(
            f"""SELECT {DATABASEKEYS.USERDATA.ETAG} FROM {DATABASEKEYS.TABLES.USERDATA} WHERE {DATABASEKEYS.USERDATA.USER_ID} = %s""",
            (user_id,),
        )
        con.commit()
    ...
    def userdata_etag_validation(
        self, user_id: int, client_etag: str
    ) -> tuple[bool, Any]:
        if not client_etag:
            return False, None
    ...
    def generate_userdata_etag(self, user_id, userdata_version):
        etag_string = self.genarate_etag_string(
            user_id=user_id, version=userdata_version
        )
